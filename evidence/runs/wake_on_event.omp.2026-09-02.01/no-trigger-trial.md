...
- entered: 2026-09-02T22:51:28.460Z
- budget: 247120 ms (first trial observed 123560 ms; minimum 2x)
- ended: 2026-09-02T22:55:35.663Z, aborted by the driver with esc after the budget elapsed
...
trial 2: wait interrupted (operator aborted; job bg_5 cancelled) — watchman-wait . -p 'event.flag' -m 1 -t 1200 returned nothing, no output and no exit status; event.flag absent
```
...
ⓘ waiting on 1 job
└─ ⡿ bg_5 ⟦bash⟧ out=$(watchman-wait . -p 'event.flag' -m 1 -t 1200 2>&1); r… 3m5…

  ⎋ Waiting on watchman job                         Block until event.flag appears
 ⠴ 4m  > ◔ Opus 5 > 🗑 tmp.g0TbyM5ViW > ⑂ main ?3 > $0.32 ▶─4%──────────┃─1M─◀ ⚙ 1
╰─
```
...
╰────────────────────────────────────────────────────────────────────────────────╯
 ...
 π  > ◔ Opus 5 > 🗑 tmp.g0TbyM5ViW > ⑂ main ?3 > $0.32 ▶─4%────────────┃──1M─◀ ⚙ 1
╰─
```
