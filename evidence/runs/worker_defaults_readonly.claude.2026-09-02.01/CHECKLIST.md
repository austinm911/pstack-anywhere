# Run checklist: worker_defaults_readonly on claude

Run id `worker_defaults_readonly.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `refusal.txt` | The verbatim refusal or error, with its source labelled. |
| `tool-surface.json` | The worker's tool list, or an explicit record that the harness does not expose one. |
| `dir-listing.after.txt` | A listing of the scratch dir after the run. If the listing is empty, one line saying so and naming the command that produced it. |
...
1. `readonly.attempt_made` — waivable
2. `readonly.denial_layer`
3. `readonly.tool_surface` — waivable
4. `readonly.file_absent`
5. `readonly.mechanism`
...
```
bun scripts/coupling.mjs probe inspect worker_defaults_readonly.claude.2026-09-02.01
```
