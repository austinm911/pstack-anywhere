# Run checklist: worker_defaults_identity on claude

Run id `worker_defaults_identity.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `definition.txt` | The verbatim installed definition, plus its install path. |
| `positive-report.txt` | The worker's verbatim first line and full report. |
| `negative-trial.txt` | Verbatim output of the poteto-absent spawn. If it produced no output, one line saying so and naming the command. |
...
1. `identity.definition_path` — waivable
2. `identity.selection_argument`
3. `identity.token_present`
4. `identity.unknown_name_behavior`
...
```
bun scripts/coupling.mjs probe inspect worker_defaults_identity.claude.2026-09-02.01
```
