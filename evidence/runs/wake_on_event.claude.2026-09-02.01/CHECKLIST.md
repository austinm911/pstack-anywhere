# Run checklist: wake_on_event on claude

Run id `wake_on_event.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `wait-accounting.yaml` | Turn count and token usage before, during, and after the wait, with the accounting source named. |
| `timeline.yaml` | Wait entry, trigger, and first post-wake action, in UTC. |
| `no-trigger-trial.md` | The second trial verbatim, including how it ended. If nothing was emitted, one line saying so and naming how the trial was ended. |
...
1. `wake.mechanism`
2. `wake.turns_during_wait` — waivable
3. `wake.latency`
4. `wake.event_kinds`
5. `wake.timeout_behavior`
...
```
bun scripts/coupling.mjs probe inspect wake_on_event.claude.2026-09-02.01
```
