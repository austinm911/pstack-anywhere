# Run checklist: probe_worker on claude

Run id `probe_worker.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `probe-activity.log` | The complete log, unedited, spanning the unprobed and probed windows. |
| `probes.yaml` | Each probe's UTC timestamp, arguments, and verbatim return. |
| `cadence.md` | The line-interval comparison between the unprobed and probed windows. |
...
1. `probe.mechanism`
2. `probe.states_reported`
3. `probe.granularity`
4. `probe.side_effect`
5. `probe.post_completion`
...
```
bun scripts/coupling.mjs probe inspect probe_worker.claude.2026-09-02.01
```
