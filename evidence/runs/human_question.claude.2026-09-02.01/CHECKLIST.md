# Run checklist: human_question on claude

Run id `human_question.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `question-render.txt` | What the operator actually saw, verbatim, for each of the three trials. |
| `answer-received.txt` | The exact value that reached the agent, plus any mapping turn. |
| `flood-trial.md` | The three completions and the pending question's state around them, in UTC order. |
...
2. `question.options_discrete`
3. `question.answer_shape`
4. `question.pending_survives_flood`
5. `question.unanswered_behavior`
...
```
bun scripts/coupling.mjs probe inspect human_question.claude.2026-09-02.01
```
