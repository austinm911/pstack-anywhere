# Run checklist: worker_durability on claude

Run id `worker_durability.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `heartbeat.log` | The complete log, unedited. |
| `termination.yaml` | Method, pid, UTC timestamp, and how the timestamp was taken. |
| `reattach-attempt.md` | The fresh session's verbatim attempt to enumerate or reach the worker, including failures. |
...
1. `durability.parent_terminated_at`
2. `durability.heartbeats_after_termination`
3. `durability.completion_marker`
4. `durability.reattach`
5. `durability.result_recoverable`
...
```
bun scripts/coupling.mjs probe inspect worker_durability.claude.2026-09-02.01
```
