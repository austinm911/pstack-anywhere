# Run checklist: worker_defaults_background on claude

Run id `worker_defaults_background.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `parent-alive.txt` | The parent's timestamp, written mid-run. |
| `worker-done.txt` | The worker's completion timestamp. |
| `timeline.yaml` | The four timestamps in one place with their sources, so the ordering claim can be checked without re-reading the transcript. |
...
1. `background.control_returned_at`
2. `background.worker_finished_at`
3. `background.parent_acted_at`
4. `background.collection_mechanism`
5. `background.result_intact`
...
```
bun scripts/coupling.mjs probe inspect worker_defaults_background.claude.2026-09-02.01
```
