# Run checklist: worker_defaults_model on claude

Run id `worker_defaults_model.claude.2026-09-02.01`. Nothing below has been done. This directory holds a
...
| `usage-record.txt` | The verbatim harness-side or provider-side model record for both trials, with its source named. |
| `self-reports.txt` | Both workers' self-reported identities, labelled non-authoritative. |
| `negative-trial.txt` | Verbatim output of the unknown-slug spawn. If it produced no output, one line saying so and naming the command. |
...
1. `model.selection_argument`
2. `model.harness_reported` — waivable
3. `model.self_report`
4. `model.selections_differed`
5. `model.unknown_slug_behavior`
...
```
bun scripts/coupling.mjs probe inspect worker_defaults_model.claude.2026-09-02.01
```
