...
Before spawning investigators, list the available MCPs from your agent harness's environment. Use the available-tools map or the harness's documented tool discovery mechanism, following `mcp_discovery` in `../poteto-mode/capabilities.md`. If discovery is unavailable, report that coverage gap.
...
- the delegate identity your harness uses (see the `identity` row in `../poteto-mode/capabilities.md`)
- `model`: your configured why-investigators model (default: the parent chat model)
- Use delegates with access to the evidence tools, following the `readonly` row in `../poteto-mode/capabilities.md`. Investigators must not write to the systems they inspect.
...
- the delegate identity your harness uses (see the `identity` row in `../poteto-mode/capabilities.md`)
- `model`: your configured why-synthesizer model (default: the parent chat model)
- Use the `readonly` row in `../poteto-mode/capabilities.md` to retain the evidence tools needed for citation checks. The synthesizer must not write to the systems it inspects.
