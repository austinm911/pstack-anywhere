...
Discover the harness's history API or session storage through the `transcript_dir` row in `../poteto-mode/capabilities.md`. Inspect its actual record format and metadata before searching. Do not assume one file per chat or one message per line. If history is unavailable, report that gap and use the context the user supplied.
...
3. Fan out across your chat history. Spawn parallel subagents on a fast, cheap model, each taking a slice of the corpus. Order candidates by the history API's timestamps or real file modification time, never by UUID name. Search the topic first and read only matching chats and their relevant regions. Skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat ID. For one or two chats, or when delegation cannot access history, search directly. The main thread receives findings instead of raw transcripts.
