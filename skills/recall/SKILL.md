...
Transcripts live in the harness's session directory (the `transcript_dir` row in `../poteto-mode/capabilities.md`), one file per chat. On upstream Cursor that is the transcripts subdirectory of `~/.cursor/projects/<slug>/`, one `<uuid>/<uuid>.jsonl` per chat, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`). Every line is one chat message.
