...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the harness's session directory (the `transcript_dir` row in `../poteto-mode/capabilities.md`, the system prompt names the path). Don't glob across the harness's other projects. That reads unrelated private chats. Walk the log against what actually happened:
