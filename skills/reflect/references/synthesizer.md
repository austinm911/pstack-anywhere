...
- Existing-skill-first: propose `new skill via skill_authoring:` only when no existing skill is a real home, the pattern recurs, and the topic deserves its own skill.
...
- "your `review_automation` tool flagged regex backtracking on May 2"
...
| <new pattern, no existing skill is a real home> | <draft a new skill via the skill_authoring rules in `../poteto-mode/playbooks/authoring-a-skill.md`> | <new skill via skill_authoring: <kebab-name>> |
