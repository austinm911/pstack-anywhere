...
4. Verify each component against its baseline via image diff on the matching surface via your UI driver. A nonzero diff is a fail. Investigate the pixel delta. Use `wake_on_event` from `capabilities.md` per component until the diff is zero.
