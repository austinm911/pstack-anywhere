...
4. Verify each component against its baseline via image diff on the matching surface via your UI driver. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Use the `wake_on_event` capability in `capabilities.md` for each component until the diff is zero.
