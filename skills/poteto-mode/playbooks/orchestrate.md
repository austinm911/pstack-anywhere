...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned through the `spawn_worker` capability, and their status is read through `probe_worker`; completion pointers are drained at drain points. See `capabilities.md` for each harness's resolution. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, with state externalized in the store, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, and a nested spawn uses the full delegate schema with `worker_defaults`). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use `spawn_worker` with `worker_defaults` for each delegate. The defaults cover background execution, read-only mode, identity, and model role. See `capabilities.md` for the per-harness parameter names. A ported `poteto-agent` definition has to exist in the harness's own subagent directory. Until it does, the identity parameter has no resolution. Use your `ui_driver` or `cli_driver` when runtime verification needs this machine. If either role is absent, name the missing runtime verification and do not claim a pass. Keep simulator use, local IDE state, and auth that exists only here on this machine. Workers without local store access need briefs that inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe `human_question` state. The file-backed gate is the portable part and matters more where the harness's question primitive degrades.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Local spawns may reference the standing-orders file by store path; verbatim paste is for spawns without store access and every resume.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the default worker placement and local exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run away from the coordinator when possible; a local restack at this scale takes the laptop down.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Use `probe_worker` for a read-only status check. OMP uses a jobs query; other harnesses degrade to job status only. Also inspect the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness.
