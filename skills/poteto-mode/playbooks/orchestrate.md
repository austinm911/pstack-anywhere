...
- Never resume an agent to check on it. A resume restarts an idle agent. Use `probe_worker` for status where available. Also inspect the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness.
