...
1. Locate the prior trail. A local transcript under the harness's own session directory (the system prompt names the user-and-harness-specific path; do not glob across unrelated workspace directories, which crosses workspace boundaries and reads private chats from unrelated projects), or a pushed branch. A cloud-agent URL handoff is unavailable on target harnesses because `worker_durability` is absent (see `capabilities.md`). If handed one from an upstream environment, respawn from the stored brief rather than reattach. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
