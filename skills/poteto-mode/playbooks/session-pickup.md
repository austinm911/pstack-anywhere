...
1. Locate the prior trail. A local transcript under the active workspace's session directory (the system prompt names the path. Do not glob across unrelated workspace directories, that crosses workspace boundaries and reads private chats from unrelated projects), a live worker's URL where the harness supports it, or a pushed branch. Check `worker_durability` in `capabilities.md` before deciding whether to resume or replace a worker. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
