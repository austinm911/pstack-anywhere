...
// Structural, not a model slug. Any model value passes; ten lanes at the PR head is the requirement.
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks "Ten lanes on <model> at the PR head"`);
