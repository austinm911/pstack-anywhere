...
1. Confirm the `Comment Sicko` delegate is available with its own instructions, then spawn it through the harness's `spawn_worker` and `identity` mappings in `../poteto-mode/capabilities.md`. If it is unavailable, report the missing reviewer and leave this review unverified. Pass the scope. Do not restate its rules.
